...
import type {
  ...
  QueryResult,
  SaveResult,
} from './issue-management/types.js';
import { localFilePlugin } from './issue-management/local-file-plugin.js';
import { jiraPlugin } from './issue-management/jira-plugin.js';
...
> & {
  ...
  issueManagementPlugin?: OrchestratorOptions['issueManagementPlugin'];
  pluginConfig?: OrchestratorOptions['pluginConfig'];
  outputFormat?: OrchestratorOptions['outputFormat'];
} = {
  maxConcurrency: 2,
  verbose: false,
...
export class StreamingReviewOrchestrator {
  ...
  private existingIssues: ValidatedIssue[] = []; // Issues from plugin.query()
  ...
  async review(input: OrchestratorInput): Promise<ReviewReport> {
    ...
    try {
      // Phase 0: Initialize Issue Management Plugin (if configured)
      if (this.options.issueManagementPlugin) {
        this.progress.phase(0, 5, '初始化 Issue Management Plugin...');

        // Select plugin instance
        this.issueManagementPlugin =
          this.options.issueManagementPlugin === 'jira' ? jiraPlugin : localFilePlugin;

        const pluginConfig: PluginConfig = this.options.pluginConfig || {};

        // Validate plugin configuration
        try {
          await this.issueManagementPlugin.validate(pluginConfig);
          this.progress.success(`插件验证成功: ${this.issueManagementPlugin.name}`);
        } catch (error) {
          this.progress.error(
            `插件验证失败: ${error instanceof Error ? error.message : String(error)}`
          );
          throw error;
        }
        ...
        try {
          const pluginContext: PluginContext = {
            ...
            sourceRef: input.sourceBranch,
            targetRef: input.targetBranch,
            branch: input.sourceBranch,
            language: 'zh',
          };

          const queryResult: QueryResult = await this.issueManagementPlugin.query(
            pluginContext,
            pluginConfig
          );
          this.existingIssues = queryResult.issues;

          this.progress.success(`查询到 ${this.existingIssues.length} 个现有 issues`);
          ...
          if (Object.keys(statusCount).length > 0) {
            const statusStr = Object.entries(statusCount)
              .map(([s, c]) => `${s}: ${c}`)
              .join(', ');
            this.progress.info(`  状态分布: ${statusStr}`);
          }
        } catch (error) {
          this.progress.warn(
            `查询现有 issues 失败: ${error instanceof Error ? error.message : String(error)}`
          );
          // Continue without existing issues
          this.existingIssues = [];
        }
      }

      // Phase 1: Build review context
      this.progress.phase(this.options.issueManagementPlugin ? 1 : 0, 4, '构建审查上下文...');
      if (this.options.verbose) {
            ...
            callbacks: {
              ...
              onIssueValidated: (issue) => {
                ...
                this.progress.issueValidated({
                  ...
                  status: issue.validation_status ?? 'pending',
                  reason,
                });
              },
      ...
      // Save issues via plugin (if configured)
      await this.saveIssuesIfNeeded(report, {
        repoPath: input.repoPath,
        sourceRef: input.sourceBranch,
        targetRef: input.targetBranch,
      });
...
Write all text (title, description, suggestion) in Chinese.`,
  ...
  private async runSegmentedReview(params: {
    ...
    // Save issues via plugin (if configured)
    await this.saveIssuesIfNeeded(report, {
      repoPath: params.reviewRepoPath,
      sourceRef: params.context.diff.sourceRef?.value || '',
      targetRef: params.context.diff.targetRef?.value || '',
    });
  ...
  /**
   * Save issues via Issue Management Plugin (if configured)
   */
  private async saveIssuesIfNeeded(
    report: ReviewReport,
    refs: { repoPath: string; sourceRef: string; targetRef?: string }
  ): Promise<void> {
    if (!this.issueManagementPlugin) {
      return;
    }

    this.progress.phase(5, 5, '保存 issues...');
    ...
    try {
      const pluginContext: PluginContext = {
        ...
        branch: refs.sourceRef,
        language: 'zh',
      };

      const pluginConfig: PluginConfig = this.options.pluginConfig || {};

      // Analyze diff to determine new and changed issues
      const diffResult = analyzeDiff(report.issues, this.existingIssues);

      this.progress.info(
        `差异分析: ${diffResult.newIssues.length} 新增, ${diffResult.matchedExisting.length} 匹配`
      );

      // Save new issues
      if (diffResult.newIssues.length > 0) {
        const saveResult: SaveResult = await this.issueManagementPlugin.save(
          diffResult.newIssues,
          [], // Changed issues not implemented yet
          pluginContext,
          pluginConfig
        );

        if (saveResult.failed.length > 0) {
          this.progress.error(`保存 ${saveResult.failed.length} 个 issues 失败`);
          for (const { issue, error } of saveResult.failed) {
            this.progress.error(`  ${issue.title}: ${error}`);
          }
        }

        const savedCount = diffResult.newIssues.length - saveResult.failed.length;
        if (savedCount > 0) {
          this.progress.success(`成功保存 ${savedCount} 个新 issues`);
        }

        // Update report with new issue IDs (for JiraPlugin which replaces UUID with key)
        if (saveResult.info?.keys) {
          const keys = saveResult.info.keys as string[];
          this.progress.info(`创建的 tickets: ${keys.join(', ')}`);
        }
      } else {
        this.progress.info('无需保存新 issues');
      }
    } catch (error) {
      this.progress.error(
        `保存 issues 失败: ${error instanceof Error ? error.message : String(error)}`
      );
      // Don't throw - still return the report
    }
  }
}
